...
export default function Navbar({ theme, onTheme, active }) {
  ...
  return <header className={`navbar ${active !== 'inicio' ? 'scrolled' : ''}`}>
    ...
    <nav id="mobile-navigation" className={open ? 'nav-links open' : 'nav-links'} aria-label="Navegação principal">
    ...
    <div className="nav-actions"><button className="theme-toggle" onClick={onTheme} aria-label={`Ativar tema ${theme === 'dark' ? 'claro' : 'escuro'}`}><span>{theme === 'dark' ? <Sun size={15} /> : <Moon size={15} />}</span><i /></button><button className="menu-toggle" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? 'Fechar menu' : 'Abrir menu'}>{open ? <X /> : <Menu />}</button></div>
  </header>;
}
